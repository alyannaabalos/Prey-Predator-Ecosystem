...
to setup
  clear-all
  ifelse netlogo-web? [ set max-mice 10000 ] [ set max-mice 30000 ]

  ifelse model-version = "mice-cats-produce" [
    ask patches [
      set pcolor green
      set countdown produce-regrowth-time
    ]
  ][ask patches[set pcolor brown]]
  create-mice initial-number-mice
  [
...
to go
  ...
  if produce-stopper [if all? patches[pcolor = brown] [stop]]
  if cat-stopper [if count cats = 0 [stop]]
  if mice-stopper [if count mice = 0 [stop]]
  if fiveH-stopper [if ticks = 500 [stop]]
  if not any? cats and count mice > max-mice [ user-message "The mice have inherited the earth" stop ]
  ask mice [
    ifelse coin-flip? [move][sharp_turn]

    if model-version = "mice-cats-produce" [
      set energy energy - 1
      eat-produce
      death
    ]
...
to eat-produce
  if pcolor = green [
    set pcolor brown
    set energy energy + mice-gain-from-food
    set countdown random produce-regrowth-time
  ]
end
...
to eat-mice
  ...
  if prey != nobody  [
    ask prey [ die ]
    set energy energy + cats-gain-from-food
  ]
end
...
"" ""
PENS
"mice" 1.0 0 -12895429 true "" "plot count mice"
"cats" 1.0 0 -4079321 true "" "plot count cats"
"produce" 1.0 0 -10899396 true "" "if model-version = \"mice-cats-produce\" [ plot count grass / 4 ]"
